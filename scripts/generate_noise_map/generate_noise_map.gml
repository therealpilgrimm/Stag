...
for (var yy = 0; yy < size; yy += 1) {
	for (var xx = 0; xx < size; xx += 1) {
		...
		if (attn1 > 0) {
			attn1 *= attn1;
			value += attn1 * attn1 * generate_simplex_extrapolate(xsb + 1, ysb + 0, dx1, dy1, perm);
		}
		...
		if (attn2 > 0) {
			attn2 *= attn2;
			value += attn2 * attn2 * generate_simplex_extrapolate(xsb + 0, ysb + 1, dx2, dy2, perm);
		}
		...
		if (attn0 > 0) {
			attn0 *= attn0;
			value += attn0 * attn0 * generate_simplex_extrapolate(xsb, ysb, dx0, dy0, perm);
		}
		...
		if (attn_ext > 0) {
			attn_ext *= attn_ext;
			value += attn_ext * attn_ext * generate_simplex_extrapolate(xsv_ext, ysv_ext, dx_ext, dy_ext, perm);
		}
...
return normalize_array_2d(map, factor);
